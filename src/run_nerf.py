...
def batchify(fn, chunk):
    ...
    def ret(inputs):
        ...
        for i in range(0, inputs.shape[0], chunk):
            output,feature=fn(inputs[i:i + chunk])
...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024 * 64):
    ...
    outputs_flat,feature = batchify(fn, netchunk)(embedded)
    # print(feature.shape)
...
def render_rays(ray_batch,
    ...
    #     raw = run_network(pts)
    raw, feature = network_query_fn(pts, viewdirs, network_fn)
    # print(feature.shape)
    rgb_map, disp_map, acc_map, weights, depth_map, rgb = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd,
                                                                      pytest=pytest)
    # print(rgb_map.shape)
    ### depth
    ret = {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map, 'depth_map': depth_map, "weights": weights,"feature" : feature.reshape(rgb_map.shape[0],64,-1)}
    ### orginal
...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt):
        d = torch.log(depth_est[depth_gt>0]) - torch.log(depth_gt[depth_gt>0])
        if torch.isnan(torch.sqrt((d ** 2).mean() - self.variance_focus * (d.mean() ** 2) + 1e-8) * 10.0):
            return None
        else:
            return torch.sqrt((d ** 2).mean() - self.variance_focus * (d.mean() ** 2) + 1e-8) * 10.0
...
def train(args):
    ...
    ### add depth loss
    gt_depths, _ = load_gt_depths(image_list, args.datadir, H, W)
    ...
    conloss=SupConLoss(temperature=0.07)
    ...
    depths_pri = np.stack([depth_priors, depth_confidences.astype(np.float32),
                           gt_depths.astype(np.float32)], -1)  # [N, H, W, 3]
    ...
    for i in trange(start, N_iters):
        ...
        target_prior = batch[3] ### target_prior[0]: pseudo label target_prior[1]: confidence target_prior[2]: gt
        i_batch += N_rand
        ...
        feature = extras['feature']
        # print(feature.shape)
        # print(target_prior[:, 0])
        # print(depth.shape)
        depth_loss= silog_criterion.forward(depth, target_prior[:, 2])
        # print(depth_loss)
        ...
        if depth_loss is None:
            ...
            # print('depth working!!!')
            loss = img_loss + 0.0005 * depth_loss
        ### unsupervised contrastive loss
        contrastive_loss = conloss(feature, depth)
        loss += 0.0005 * contrastive_loss
