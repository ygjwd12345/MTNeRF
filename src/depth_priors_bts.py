...
from models.transdepth.bts import BtsModel
...
def create_depth_model(args):
    ...
    depth_model = BtsModel(args)
    ### load pretrained model
    checkpoint = torch.load('./models/transdepth/checkpoint/nyu_depth',map_location='cuda:0')
    depth_model.load_state_dict(checkpoint['model'],strict= False)
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        ### bts
        depth_model.load_state_dict(ckpt['netG_state_dict'])
        ###
...
def train(args):
    ...
    images_train = images.clone().to(device)
    ...
    # focal=tensor.new_full((images.shape[0],1), 606.5173)
    # focal_train = focal.to(device)
    ...
    for i in trange(start, N_iters_depth):
        ...
        ### bts
        # depth_pred = depth_model(batch,focal_train[i_batch:i_batch + N_rand_depth])
        ...
        if i % args.depth_i_weights==0:
            ...
            # torch.save({
            #     'global_step': i,
            #     'netG_state_dict': depth_model.state_dict(),
            #     'optimizer_state_dict': optimizer_depth.state_dict(),
            # }, path)
    ...
    with torch.no_grad():
        ...
        for i, image_name in enumerate(image_list):
            ...
            batch = images[i:i + 1]
            depth_pred = depth_model.forward(batch.to(device),focal_train[i:i + 1])
