...
def cal_depth_confidences(depths, T, K, i_train, topk=4):
    ...
    for i in range(depths.shape[0]):
        ...
        depth_confidence =  depth_confidence.mean(0)
        depth_confidence =  depth_confidence.numpy()
...
def BackprojectDepth(depth, invK, pix_coords):
    ...
    cam_points = torch.matmul(invK[:, :3, :3], pix_coords).cpu()
    cam_points = depth.view(batch_size, 1, -1) * cam_points
    cam_points = torch.cat([cam_points, ones.cpu()], 1)
    return cam_points
...
def Project3D(points, K, T, H, W, eps=1e-7):
    ...
    cam_points = torch.matmul(P.cpu(), points.cpu())
